...
typedef enum {
  ...
  /**
   * @brief the depth precision level, which may change the depth frame data unit, needs to be
   * confirmed through the ValueScale interface of DepthFrame
   */
  ...
  /**
   * @brief brt2r-rectify function switch (brt2r is a special module on mx6600), 0: Disable, 1:
   * Rectify Enable
   */
  ...
  /**
   * @brief Disparity to depth switch, false: switch to software disparity convert to depth, true:
   * switch to hardware disparity convert to depth
   */
  ...
  /**
   * @brief Custom RGB cropping switch, 0 is off, 1 is on custom cropping, and the ROI cropping area
   * is issued
   */
  ...
  /**
   * @brief Number frame to capture once a 'OB_PROP_CAPTURE_IMAGE_SIGNAL_BOOL' effect. range: [1,
   * 255]
   */
  ...
  /**
   * @brief Get hardware laser power actual level which real state of laser element.
   * OB_PROP_LASER_POWER_LEVEL_CONTROL_INT99 will effect this command which it setting and changed
   * the hardware laser energy level.
   */
  ...
  /**
   * @brief Device development mode switch, optional modes can refer to the definition in @ref
   * OBDeviceDevelopmentMode,the default mode is
   * @ref OB_USER_MODE
  ...
  /**
   * @brief Multi-DeviceSync synchronized signal trigger out is enable state. true: enable, false:
   * disable
   */
   ...
   * @attention This command can only be written, and the parameter value must be true. The command
   * takes effect after restarting the device.
   */
  OB_PROP_RESTORE_FACTORY_SETTINGS_BOOL = 131,

  /**
   * @brief Enter recovery mode (flashing mode) when boot the device
   * @attention The device will take effect after rebooting with the enable option. After entering
   * recovery mode, you can upgrade the device system. Upgrading the system may cause system damage,
  ...
  /**
   * @brief Enable or disable the device to retry USB2.0 re-identification when the device is
   * connected to a USB2.0 port.
   * @brief This feature ensures that the device is not mistakenly identified as a USB 2.0 device
  ...
  /**
   * @brief Laser always on, true: always on, false: off, laser will be turned off when out of
   * exposure time
   */
   ...
   * @brief This property allows continuous adjustment of the depth unit, unlike @ref
   * OB_PROP_DEPTH_PRECISION_LEVEL_INT must be set to some fixed value.
   */
   ...
   *
   * @brief Currently using for DabaiA device,if the exposure value is lower than a certain
   * threshold, the laser is turned off; if it exceeds another threshold, the laser is turned on
   * again.
   */
  ...
  /**
   * @brief soft trigger auto capture enable, use in OB_MULTI_DEVICE_SYNC_MODE_SOFTWARE_TRIGGERING
   * mode
   */
  OB_DEVICE_AUTO_CAPTURE_ENABLE_BOOL = 216,
  /**
   * @brief soft trigger auto capture interval time, use in
   * OB_MULTI_DEVICE_SYNC_MODE_SOFTWARE_TRIGGERING mode
   */
  ...
  /**
   * @brief Intra-camera Sync Reference based on the exposure start time, the exposure middle time,
   * or the exposure end time. the definition in @ref OBIntraCameraSyncReference
   */
  ...
  /**
   * @brief A list of depth accuracy levels, returning an array of uin16_t, corresponding to the
   * enumeration
   */
   ...
   * @brief Using for get last static ip config, witch is record in device flash when user set
   * static ip config
   *
   * @attention read only
   */
   ...
   * @brief The Value type is @ref OBRegionOfInterest
   * @brief Since the ir sensor is the same physical sensor as the depth sensor, this property will
   * also effect the ir sensor.
   */
  ...
  /**
   * @brief Automatic exposure of depth camera (infrared camera will be set synchronously under some
   * models of devices)
   */
  OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL = 2016,

  /**
   * @brief Depth camera exposure adjustment (infrared cameras will be set synchronously under some
   * models of devices)
   */
  OB_PROP_DEPTH_EXPOSURE_INT = 2017,

  /**
   * @brief Depth camera gain adjustment (infrared cameras will be set synchronously under some
   * models of devices)
   */
  OB_PROP_DEPTH_GAIN_INT = 2018,

  /**
   * @brief Infrared camera auto exposure (depth camera will be set synchronously under some models
   * of devices)
   */
  OB_PROP_IR_AUTO_EXPOSURE_BOOL = 2025,

  /**
   * @brief Infrared camera exposure adjustment (some models of devices will set the depth camera
   * synchronously)
   */
  OB_PROP_IR_EXPOSURE_INT = 2026,

  /**
   * @brief Infrared camera gain adjustment (the depth camera will be set synchronously under some
   * models of devices)
   */
  OB_PROP_IR_GAIN_INT = 2027,

  /**
   * @brief Select Infrared camera data source channel. If not support throw exception. 0 : IR
   * stream from IR Left sensor; 1 : IR stream from IR Right sensor;
   */
  OB_PROP_IR_CHANNEL_DATA_SOURCE_INT = 2028,

  /**
   * @brief Depth effect dedistortion, true: on, false: off. mutually exclusive with D2C function,
   * RM_Filter disable When hardware or software D2C is enabled.
   */
  ...
  /**
   * @brief The enable/disable switch for IR short exposure function, supported only by a few
   * devices.
   */
  ...
  /**
   * @brief Setting and getting the USB device frame skipping mode status, true: frame skipping
   * mode, false: non-frame skipping mode.
   */
  ...
  /**
   * @brief Depth data unpacking function switch (each open stream will be turned on by default,
   * support RLE/Y10/Y11/Y12/Y14 format)
   */
  OB_PROP_SDK_DEPTH_FRAME_UNPACK_BOOL = 3007,

  /**
   * @brief IR data unpacking function switch (each current will be turned on by default, support
   * RLE/Y10/Y11/Y12/Y14 format)
   */
  ...
  /**
   * @brief Left IR frame data unpacking function switch (each current will be turned on by default,
   * support RLE/Y10/Y11/Y12/Y14 format)
   */
  OB_PROP_SDK_IR_LEFT_FRAME_UNPACK_BOOL = 3011,

  /**
   * @brief Right IR frame data unpacking function switch (each current will be turned on by
   * default, support RLE/Y10/Y11/Y12/Y14 format)
   */
  ...
  /**
   * @brief Read the current network bandwidth type of the network device, whether it is Gigabit
   * Ethernet or Fast Ethernet, such as G335LE.
   */
  OB_PROP_NETWORK_BANDWIDTH_TYPE_INT = 3027,

  /**
   * @brief Switch device performance mode, currently available in Adaptive Mode and High
   * Performance Mode, such as G335LE.
   */
  ...
  /**
   * @brief Color camera CCI denoising level. 0: Auto; 1-8: higher values indicate stronger
   * denoising.
