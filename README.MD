...
For Femto Mega and Femto Bolt devices, the depth NFOV and WFOV modes are achieved by configuring different resolutions in the depth startup parameters.The depth startup parameters depth_width and depth_height specify the resolution of the depth stream, the frame fps and resolution of the IR need to be consistent with the depth. The corresponding resolutions for different modes are as follows:
