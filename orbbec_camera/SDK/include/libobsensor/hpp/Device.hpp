...
class Device {
 ...
 public:
  ...
  ob_device_t *getImpl() const { return impl_; }
   ...
   *
   * @return std::shared_ptr<Sensor> return the sensor example, if the device does not have the
  ...
  void updateFirmware(const char *filePath, DeviceFwUpdateCallback callback, bool async = true) {
    ...
    ob_device_update_firmware(impl_, filePath, &Device::firmwareUpdateCallback, async, this,
                              &error);
    Error::handle(&error);
  }
   ...
   * @param[in] async Whether to execute asynchronously
   */
  void updateFirmwareFromData(const uint8_t *firmwareData, uint32_t firmwareDataSize,
                              DeviceFwUpdateCallback callback, bool async = true) {
    ob_error *error = nullptr;
    fwUpdateCallback_ = callback;
    ob_device_update_firmware_from_data(impl_, firmwareData, firmwareDataSize,
                                        &Device::firmwareUpdateCallback, async, this, &error);
    Error::handle(&error);
  }
   ...
   *
   * @param[in] filePathList A list(2D array) of preset file paths, each up to OB_PATH_MAX
   * characters.
   * @param[in] pathCount The number of the preset file paths.
   * @param[in] callback Preset update progress and status callback
   */
  void updateOptionalDepthPresets(const char filePathList[][OB_PATH_MAX], uint8_t pathCount,
                                  DeviceFwUpdateCallback callback) {
    ob_error *error = nullptr;
    fwUpdateCallback_ = callback;
    ob_device_update_optional_depth_presets(impl_, filePathList, pathCount,
                                            &Device::firmwareUpdateCallback, this, &error);
    Error::handle(&error);
  }
   ...
   *
   * @param[in] callback The callback function that is triggered when the device status changes (for
   * example, the frame rate is automatically reduced or the stream is closed due to high
   * temperature, etc.)
   */
  ...
  /**
   * @brief Switch depth work mode by OBDepthWorkMode. Prefer invoke switchDepthWorkMode(const char
   * *modeName) to switch depth mode when known the complete name of depth work mode.
   * @param[in] workMode Depth work mode come from ob_depth_work_mode_list which return by
   ...
   * @attention The device will be disconnected and reconnected. After the device is disconnected,
   * the access to the Device object interface may be abnormal. Please delete the object directly
   * and obtain it again after the device is reconnected.
   */
  void reboot() const {
    ob_error *error = nullptr;
   ...
   * @attention The device will be disconnected and reconnected. After the device is disconnected,
   * the access to the Device object interface may be abnormal. Please delete the object directly
   * and obtain it again after the device is reconnected. Support devices: Gemini2 L
   *
   * @param[in] delayMs Time unit: ms. delayMs == 0: No delay; delayMs > 0, Delay millisecond
   * connect to host device after reboot
   */
   ...
   * @brief After enable the device heartbeat, the sdk will start a thread to send heartbeat signal
   * to the device error every 3 seconds.
   *
   ...
   * @brief For example, if the return value is 0b00001100, it means the device supports @ref
   * OB_MULTI_DEVICE_SYNC_MODE_PRIMARY and @ref OB_MULTI_DEVICE_SYNC_MODE_SECONDARY. User can check
   * the supported mode by the code:
   * ```c
   ...
   * @brief The device will start one time image capture after receiving the capture command when it
   * is in the @ref OB_MULTI_DEVICE_SYNC_MODE_SOFTWARE_TRIGGERING
   *
   * @attention The frequency of the user call this function multiplied by the number of frames per
   * trigger should be less than the frame rate of the stream. The number of frames per trigger can
   * be set by @ref OBMultiDeviceSyncConfig::framesPerTrigger.
   * @attention For some models, receive and execute the capture command will have a certain delay
   * and performance consumption, so the frequency of calling this function should not be too high,
   * please refer to the product manual for the specific supported frequency.
   * @attention If the device is not in the @ref OB_MULTI_DEVICE_SYNC_MODE_HARDWARE_TRIGGERING mode,
   * device will ignore the capture command.
   */
   ...
   * @brief The device will reset the timer for calculating the timestamp for output frames to 0
   * after receiving the timestamp reset command when the timestamp reset function is enabled. The
   * timestamp reset function can be enabled by call @ref ob_device_set_timestamp_reset_config.
   * @brief Before calling this function, user should call @ref ob_device_set_timestamp_reset_config
   * to disable the timestamp reset function (It is not required for some models, but it is still
   * recommended to do so for code compatibility).
   *
   * @attention If the stream of the device is started, the timestamp of the continuous frames
   * output by the stream will jump once after the timestamp reset.
   * @attention Due to the timer of device is not high-accuracy, the timestamp of the continuous
   * frames output by the stream will drift after a long time. User can call this function
   * periodically to reset the timer to avoid the timestamp drift, the recommended interval time is
   * 60 minutes.
   */
   ...
   */
  inline void timerReset() const { timestampReset(); }

  /**
   * @brief synchronize the timer of the device with the host.
   * @brief After calling this function, the timer of the device will be synchronized with the host.
   * User can call this function to multiple devices to synchronize all timers of the devices.
   *
   * @attention If the stream of the device is started, the timestamp of the continuous frames
   * output by the stream will may jump once after the timer sync.
   * @attention Due to the timer of device is not high-accuracy, the timestamp of the continuous
   * frames output by the stream will drift after a long time. User can call this function
   * periodically to synchronize the timer to avoid the timestamp drift, the recommended interval
   * time is 60 minutes.
   *
   */
   ...
   * @brief The preset mean a set of parameters or configurations that can be applied to the device
   * to achieve a specific effect or function.
   *
   * @return const char* return the current preset name, it should be one of the preset names
   * returned by @ref getAvailablePresetList.
   */
   ...
   * @attention After loading the preset, the settings in the preset will set to the device
   * immediately. Therefore, it is recommended to re-read the device settings to update the user
   * program temporarily.
   * @param[in] presetName The preset name to set. The name should be one of the preset names
   * returned by @ref getAvailablePresetList.
   */
   ...
   * @brief The available preset list usually defined by the device manufacturer and restores on the
   * device.
   * @brief User can load the custom preset by calling @ref loadPresetFromJsonFile to append the
   ...
   * @brief After loading the custom preset, the settings in the custom preset will set to the
   * device immediately.
   * @brief After loading the custom preset, the available preset list will be appended with the
   * custom preset and named as the file name.
   *
   * @attention The user should ensure that the custom preset file is adapted to the device and the
   * settings in the file are valid.
   * @attention It is recommended to re-read the device settings to update the user program
   * temporarily after successfully loading the custom preset.
   *
   ...
   * device immediately.
   * @brief After loading the custom preset, the available preset list will be appended with the
   * custom preset and named as the @p presetName.
   *
   * @attention The user should ensure that the custom preset data is adapted to the device and the
   * settings in the data are valid.
   * @attention It is recommended to re-read the device settings to update the user program
   * temporarily after successfully loading the custom preset.
   *
   ...
   * @brief After exporting the preset, a new preset named as the @p presetName will be added to the
   * available preset list.
   *
   * @attention The memory of the data is allocated by the SDK, and will automatically be released
   * by the SDK.
   * @attention The memory of the data will be reused by the SDK on the next call, so the user
   * should copy the data to a new buffer if it needs to be preserved.
   *
   * @param[in] presetName The name of the custome preset.
   * @param[out] data return the preset json data.
   * @param[out] dataSize return the size of the preset json data.
   */
  void exportSettingsAsPresetJsonData(const char *presetName, const uint8_t **data,
                                      uint32_t *dataSize) {
    ob_error *error = nullptr;
    ob_device_export_current_settings_as_preset_json_data(impl_, presetName, data, dataSize,
                                                          &error);
  }

  /**
   * @brief Export current device settings as a preset json file.
   * @brief The exported preset file can be loaded by calling @ref loadPresetFromJsonFile to restore
   * the device setting.
   * @brief After exporting the preset, a new preset named as the @p filePath will be added to the
   * available preset list.
   *
   ...
   * @brief This is a factory and debug function, which can be used to send and receive data from
   * the device. The data format is secret and belongs to the device vendor.
   *
   * @attention The send and receive data buffer are managed by the caller, the receive data buffer
   * should be allocated at 1024 bytes or larger.
   *
   ...
   * @param[in,out] receiveDataSize The requeseted size of the data received from the device, and
   * the actual size of the data received from the device.
   */
  void sendAndReceiveData(const uint8_t *sendData, uint32_t sendDataSize, uint8_t *receiveData,
                          uint32_t *receiveDataSize) const {
    ob_error *error = nullptr;
    ob_device_send_and_receive_data(impl_, sendData, sendDataSize, receiveData, receiveDataSize,
                                    &error);
    Error::handle(&error);
  }
   ...
   * @param[in] frameInterleaveName The frame interleave name to set. The name should be one of the
   * frame interleave names returned by @ref getAvailableFrameInterleaveList.
   */
 ...
 private:
  static void firmwareUpdateCallback(ob_fw_update_state state, const char *message, uint8_t percent,
                                     void *userData) {
    auto device = static_cast<Device *>(userData);
 ...
 public:
  ...
  void deviceUpgradeFromData(const uint8_t *firmwareData, uint32_t firmwareDataSize,
                             DeviceFwUpdateCallback callback, bool async = true) {
...
class DeviceInfo {
 ...
 public:
   ...
   *
   * @return const char* the connection type of the device, currently supports: "USB", "USB1.0",
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  const char *getDevicegateway() { return getDeviceGateway(); }

  const char *name() const { return getName(); }

  int pid() const { return getPid(); }

  int vid() const { return getVid(); }

  const char *uid() const { return getUid(); }

  const char *serialNumber() const { return getSerialNumber(); }

  const char *firmwareVersion() const { return getFirmwareVersion(); }

  const char *connectionType() const { return getConnectionType(); }

  const char *ipAddress() const { return getIpAddress(); }

  const char *hardwareVersion() const { return getHardwareVersion(); }

  const char *supportedMinSdkVersion() const { return getSupportedMinSdkVersion(); }

  const char *asicName() const { return getAsicName(); }

  OBDeviceType deviceType() const { return getDeviceType(); }
};
...
class DeviceList {
 ...
 public:
   ...
   *
   * @return const char* returns connection type, currently supports: "USB", "USB1.0", "USB1.1",
   * "USB2.0", "USB2.1", "USB3.0", "USB3.1", "USB3.2", "Ethernet", "GMSL2"
   */
  ...
  /**
   * @brief Get the MAC address of the host network interface corresponding to the device at the
   * specified index
   *
  ...
  /**
   * @brief Get the IP address of the host network interface corresponding to the device at the
   * specified index
   *
   * @attention Only valid for network devices, otherwise it will return "0.0.0.0".
   *
  ...
  /**
   * @brief Get the subnet length of the host network interface corresponding to the device at the
   * specified index
   *
   ...
   * @param[in] index The index of the device
   *
   * @return uint8_t The subnet length (0~32) of the host network interface associated with the
   * device.
   */
  ...
  /**
   * @brief Get the gateway of the host network interface corresponding to the device at the
   * specified index
   *
   * @attention Only valid for network devices, otherwise it will return "0.0.0.0".
   *
   ...
   *
   * @attention If the device has already been acquired and created elsewhere, repeated acquisition
  ...
  std::shared_ptr<Device> getDevice(
      uint32_t index, OBDeviceAccessMode accessMode = OB_DEVICE_DEFAULT_ACCESS) const {
    ob_error *error = nullptr;
   ...
   * @return std::shared_ptr<Device> the device object
   */
  std::shared_ptr<Device> getDeviceBySN(
      const char *serialNumber, OBDeviceAccessMode accessMode = OB_DEVICE_DEFAULT_ACCESS) const {
    ob_error *error = nullptr;
    auto device =
        ob_device_list_get_device_by_serial_number_ex(impl_, serialNumber, accessMode, &error);
    Error::handle(&error);
    return std::make_shared<Device>(device);
  }
   ...
   */
  std::shared_ptr<Device> getDeviceByUid(
      const char *uid, OBDeviceAccessMode accessMode = OB_DEVICE_DEFAULT_ACCESS) const {
    ob_error *error = nullptr;
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  uint32_t deviceCount() const { return getCount(); }

  int pid(uint32_t index) const { return getPid(index); }

  int vid(uint32_t index) const { return getVid(index); }

  const char *uid(uint32_t index) const { return getUid(index); }

  const char *serialNumber(uint32_t index) const { return getSerialNumber(index); }

  const char *name(uint32_t index) const { return getName(index); }

  const char *connectionType(uint32_t index) const { return getConnectionType(index); }

  const char *ipAddress(uint32_t index) const { return getIpAddress(index); }
};
...
class OBDepthWorkModeList {
 ...
 public:
   ...
   * @return OBDepthWorkMode the OBDepthWorkMode object at the specified index
   */
  OBDepthWorkMode operator[](uint32_t index) { return getOBDepthWorkMode(index); }

 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  uint32_t count() { return getCount(); }
};

/**
 * @brief Class representing a list of device presets
 * @brief A device preset is a set of parameters or configurations that can be applied to the device
 * to achieve a specific effect or function.
 */
class DevicePresetList {
 private:
 ...
 public:
   ...
   *
   * @return bool Returns true if the special name is found in the preset list, otherwise returns
   * false.
   */
...
class DeviceFrameInterleaveList {
 ...
 public:
   ...
   *
   * @return bool Returns true if the special name is found in the frame interleave list, otherwise
   * returns false.
   */
