...
class StreamProfile : public std::enable_shared_from_this<StreamProfile> {
 ...
 public:
  ...
  const ob_stream_profile *getImpl() const { return impl_; }
  ...
  /**
   * @brief Get the extrinsic parameters from current stream profile to the given target stream
   * profile.
   *
  ...
  OBExtrinsic getExtrinsicTo(std::shared_ptr<StreamProfile> target) const {
    ob_error *error = nullptr;
    auto extrinsic = ob_stream_profile_get_extrinsic_to(
        impl_, const_cast<ob_stream_profile_t *>(target->getImpl()), &error);
    Error::handle(&error);
    return extrinsic;
  }

  /**
   * @brief Set the extrinsic parameters from current stream profile to the given target stream
   * profile.
   *
  ...
  void bindExtrinsicTo(std::shared_ptr<StreamProfile> target, const OBExtrinsic &extrinsic) {
    ob_error *error = nullptr;
    ob_stream_profile_set_extrinsic_to(const_cast<ob_stream_profile_t *>(impl_),
                                       const_cast<const ob_stream_profile_t *>(target->getImpl()),
                                       extrinsic, &error);
    Error::handle(&error);
  }
  ...
  void bindExtrinsicTo(const OBStreamType &targetStreamType, const OBExtrinsic &extrinsic) {
    ob_error *error = nullptr;
    ob_stream_profile_set_extrinsic_to_type(const_cast<ob_stream_profile_t *>(impl_),
                                            targetStreamType, extrinsic, &error);
    Error::handle(&error);
  }
   ...
   *
   * @return std::shared_ptr<T> Return the result. Throws an exception if conversion is not
   * possible.
   */
  template <typename T>
  std::shared_ptr<T> as() {
  ...
  OBFormat format() const { return getFormat(); }

  OBStreamType type() const { return getType(); }
...
class VideoStreamProfile : public StreamProfile {
 public:
  ...
  void setIntrinsic(const OBCameraIntrinsic &intrinsic) {
    ob_error *error = nullptr;
    ob_video_stream_profile_set_intrinsic(const_cast<ob_stream_profile_t *>(impl_), intrinsic,
                                          &error);
    Error::handle(&error);
  }
  ...
  void setDistortion(const OBCameraDistortion &distortion) {
    ob_error *error = nullptr;
    ob_video_stream_profile_set_distortion(const_cast<ob_stream_profile_t *>(impl_), distortion,
                                           &error);
    Error::handle(&error);
  }
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  uint32_t fps() const { return getFps(); }
...
class AccelStreamProfile : public StreamProfile {
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  OBAccelFullScaleRange fullScaleRange() const { return getFullScaleRange(); }

  OBAccelSampleRate sampleRate() const { return getSampleRate(); }
};
...
class GyroStreamProfile : public StreamProfile {
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  OBGyroFullScaleRange fullScaleRange() const { return getFullScaleRange(); }

  OBGyroSampleRate sampleRate() const { return getSampleRate(); }
};
 ...
 public:
  static std::shared_ptr<StreamProfile> create(const ob_stream_profile_t *impl) {
    ...
    switch (type) {
      ...
      default: {
        ob_error *err =
            ob_create_error(OB_STATUS_ERROR, "Unsupported stream type.",
                            "StreamProfileFactory::create", "", OB_EXCEPTION_TYPE_INVALID_VALUE);
        Error::handle(&err);
        return nullptr;
      }
    }
...
class StreamProfileList {
 ...
 public:
   ...
   *
   * @param[in] index The index of the StreamProfile object to be retrieved. Must be in the range
   * [0, count-1]. Throws an exception if the index is out of range.
   *
  ...
  /**
   * @brief Match the corresponding video stream profile based on the passed-in parameters. If
   * multiple Match are found, the first one in the list is returned by default. Throws an exception
   * if no matching profile is found.
   *
   * @param[in] width The width of the stream. Pass OB_WIDTH_ANY if no matching condition is
   * required.
   * @param[in] height The height of the stream. Pass OB_HEIGHT_ANY if no matching condition is
   * required.
   * @param[in] format The type of the stream. Pass OB_FORMAT_ANY if no matching condition is
   * required.
   * @param[in] fps The frame rate of the stream. Pass OB_FPS_ANY if no matching condition is
   * required.
   *
   * @return std::shared_ptr<VideoStreamProfile> Return the matching resolution.
   */
  std::shared_ptr<VideoStreamProfile> getVideoStreamProfile(int width = OB_WIDTH_ANY,
                                                            int height = OB_HEIGHT_ANY,
                                                            OBFormat format = OB_FORMAT_ANY,
                                                            int fps = OB_FPS_ANY) const {
    ob_error *error = nullptr;
    auto profile =
        ob_stream_profile_list_get_video_stream_profile(impl_, width, height, format, fps, &error);
    Error::handle(&error);
  ...
  /**
   * @brief Match the corresponding accelerometer stream profile based on the passed-in parameters.
   * If multiple Match are found, the first one in the list is returned by default. Throws an
   * exception if no matching profile is found.
   *
   ...
   */
  std::shared_ptr<AccelStreamProfile> getAccelStreamProfile(OBAccelFullScaleRange fullScaleRange,
                                                            OBAccelSampleRate sampleRate) const {
    ob_error *error = nullptr;
    auto profile =
        ob_stream_profile_list_get_accel_stream_profile(impl_, fullScaleRange, sampleRate, &error);
    Error::handle(&error);
  ...
  /**
   * @brief Match the corresponding gyroscope stream profile based on the passed-in parameters. If
   * multiple Match are found, the first one in the list is returned by default. Throws an exception
   * if no matching profile is found.
   *
   * @param[in] fullScaleRange The full scale range. Pass 0 if no matching condition is required.
   * @param[in] sampleRate The sampling frequency. Pass 0 if no matching condition is required.
   */
  std::shared_ptr<GyroStreamProfile> getGyroStreamProfile(OBGyroFullScaleRange fullScaleRange,
                                                          OBGyroSampleRate sampleRate) const {
    ob_error *error = nullptr;
    auto profile =
        ob_stream_profile_list_get_gyro_stream_profile(impl_, fullScaleRange, sampleRate, &error);
    Error::handle(&error);
  ...
  /**
   * @brief Match the corresponding LiDAR stream profile based on the passed-in parameters. If
   * multiple Match are found, the first one in the list is returned by default. Throws an exception
   * if no matching profile is found.
   *
   * @param[in] scanRate The scan rate of LiDAR. Pass OB_LIDAR_SCAN_ANY if no matching condition is
   * required.
   * @param[in] format The type of the stream. Pass OB_FORMAT_ANY if no matching condition is
   * required.
   */
  std::shared_ptr<LiDARStreamProfile> getLiDARStreamProfile(OBLiDARScanRate scanRate,
 ...
 public:
  // The following interfaces are deprecated and are retained here for compatibility purposes.
  uint32_t count() const { return getCount(); }
};
